...
static event_response_t win_cb(drakvuf_t drakvuf, drakvuf_trap_info_t* info)
{
    ...
    switch (s->format)
    {
        case OUTPUT_JSON:
        {
            ...
            printf("{\"syscall\": {\"ts\":\"%s\",\"pid\":%d,\"proc_name\":\"%s\",\"userid\":%" PRIi64 ",\"module\":\"%s\",\"trap_name\":\"%s\"",
                   mbstr, info->proc_data.pid,info->proc_data.name, info->proc_data.userid,info->trap->breakpoint.module, info->trap->name);
